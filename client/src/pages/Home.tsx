...
export default function Home() {
  const { data: featuredPosts, isLoading: loadingFeatured } = trpc.posts.getFeatured.useQuery();
        ...
        <section className="container py-6">
          {loadingFeatured ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {/* Main hero post */}
              {heroPost && (
                <Link href={`/artigo/${heroPost.slug}`} className="lg:col-span-2">
                  <article className="group cursor-pointer relative overflow-hidden rounded-lg bg-card border border-border hover:border-primary/50 transition-all duration-300 h-full">
                    <div className="aspect-video overflow-hidden">
                      <img
                        src={heroPost.featuredImage || "https://images.unsplash.com/photo-1580130732478-4e339fb33746?w=1200&q=80"}
                        alt={heroPost.title}
                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                        onError={(e) => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1580130732478-4e339fb33746?w=1200&q=80"; }}
                      />
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                    <div className="absolute bottom-0 left-0 right-0 p-5">
                      <Badge className="mb-2 bg-primary text-primary-foreground text-xs">Destaque</Badge>
                      <h1 className="text-xl md:text-2xl font-bold text-white leading-snug group-hover:text-primary transition-colors">
                        {heroPost.title}
                      </h1>
                      {heroPost.excerpt && (
                        <p className="text-sm text-white/80 mt-1 line-clamp-2">{heroPost.excerpt}</p>
                      )}
          ...
          <section className="border-y border-border bg-card/50">
            <div className="container py-3">
              <div className="flex items-center gap-2 overflow-x-auto pb-1">
                <span className="text-xs text-muted-foreground shrink-0 font-medium uppercase tracking-wider">Categorias:</span>
                {categories.map((cat) => (
                  <Link key={cat.id} href={`/categoria/${cat.slug}`}>
                    <Badge variant="outline" className="shrink-0 hover:bg-primary/20 hover:text-primary hover:border-primary/50 transition-colors cursor-pointer text-xs">
                      {cat.name}
                    </Badge>
                  </Link>
                ))}
            ...
            <div className="lg:col-span-3">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-bold text-foreground">Últimas Publicações</h2>
                </div>
            ...
            <aside className="lg:col-span-1">
              ...
              <div className="bg-card border border-border rounded-lg p-4 mb-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-sm text-foreground">Sobre o Site</h3>
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Cenas de Combate é um portal dedicado à história militar, conflitos mundiais e geopolítica. Mais de 300 artigos sobre guerras, tecnologia militar e personagens históricos.
                </p>
              </div>
