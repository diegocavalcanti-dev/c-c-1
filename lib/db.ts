...
export async function getPublishedPosts(opts: {
  ...
  if (categorySlug) {
    ...
    conditions.push(inArray(posts.id, postIds.map((p: any) => p.postId)));
  }
...
export async function createPost(
  ...
  if (insertId && categoryIds.length > 0) {
    await db
      ...
      .values(categoryIds.map((cid) => ({ postId: insertId, categoryId: cid })));
  }
  return insertId;
}
...
export async function bulkInsertPosts(
  postsData: Array<InsertPost & { categoryIds: number[] }>
) {
  const db = await getDb();
  if (!db) throw new Error("DB not available");
    ...
    try {
      ...
      if (insertId && categoryIds.length > 0) {
        ...
        await db
          ...
          .where(eq(postCategories.postId, insertId));
        await db
          .insert(postCategories)
          .values(categoryIds.map((cid) => ({ postId: insertId, categoryId: cid })));
      }
